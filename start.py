...
def q2():
	...
	for i in range(threshold10):
		for j in range(threshold10):
			if(tab1[i][0] == tab2[j][0]):
				...
				moyenne10Complexity += float(tab1[i][-2])
				moyenne10CBO += float(tab1[i][-1])
				break
	for i in range(threshold15):
		for j in range(threshold15):
			if(tab1[i][0] == tab2[j][0]):
				temptab2.append(tab1[i])
				moyenne15Complexity += float(tab1[i][-2])
				moyenne15CBO += float(tab1[i][-1])
				break
	...
	#print(f"en moyenne les fichiers ont une complexity de  : {round(moyenneMcCabe,4)}" )
	print(f"à un seuil de 10% de CBO les fichiers ont une complexite de  : {round(moyenne10Complexity,4)}" )
	print(f"à un seuil de 15% de CBO les fichiers ont une complexite de  : {round(moyenne15Complexity,4)}" )
	print(f"à un seuil  10% de complexite les fichiers ont un CBO de  : {round(moyenne10CBO,4)}" )
	print(f"à un seuil  15% de complexite les fichiers ont un CBO de  : {round(moyenne15CBO,4)}" )
